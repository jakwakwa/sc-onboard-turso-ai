...
export const accountantLetterSchema = z.object({
	...
	accountantName: z.string().min(2, "Accountant/auditor name is required"),
	...
	signatureDate: z.string().min(2, "Date is required"),
});
...
export const callCentreApplicationSchema = z.object({
	...
	serviceAgreementAccepted: z.literal(true, {
		message: "You must accept the service agreement terms",
	}),
	// Product Description
	...
	supplierEmail: z.string().email("Valid supplier email is required"),
	supplierAddress: z.string().min(5, "Supplier address is required"),
	// Call Script
	callScript: z.string().min(20, "Call script must be at least 20 characters"),
	// Signature
	signatureName: z.string().min(2, "Signature is required"),
	signatureDate: z.string().min(2, "Date is required"),
});

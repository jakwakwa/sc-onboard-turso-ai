...
export function extractBankStatementSections(text: string): {
	...
	const accountInfoMatch = cleanedText.match(
		/(?:account|holder|number|branch).*?(?=statement|transaction)/is,
	);
	const transactionsMatch = cleanedText.match(
		/(?:date|description|debit|credit|balance).*$/is,
	);
	const summaryMatch = cleanedText.match(
		/(?:closing|balance|total|summary).*$/is,
	);

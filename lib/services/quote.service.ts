...
async function generateQuoteWithAI(leadData: LeadRow, workflowId: number): Promise<QuoteGenerationResult> {
    console.log(`[QuoteService] Running AI quote generation for workflow ${workflowId}`);
...
function generateMockQuote(leadData: LeadRow): QuoteGenerationResult {
    ...
    return {
        ...
        riskFactors: itcScore < 650 ? ['Lower ITC score indicates elevated credit risk.'] : [],
...
async function notifyQuoteGenerated({
    ...
    try {
        ...
        if (!response.ok) {
            console.warn('[QuoteService] Callback returned non-200:', response.status, response.statusText);
        }
    } catch (error) {
